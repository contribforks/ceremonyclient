...
func NewDebugNode(configConfig *config.Config, selfTestReport *protobufs.SelfTestReport) (*Node, error) {
	...
	pebbleHypergraphStore := store.NewPebbleHypergraphStore(dbConfig, mdbxdb, zapLogger)
...
func NewStrictSyncNode(configConfig *config.Config, selfTestReport *protobufs.SelfTestReport, synchronizer rpc.Synchronizer) (*Node, error) {
	...
	keyConfig := configConfig.Key
	fileKeyManager := keys.NewFileKeyManager(keyConfig, zapLogger)
	node, err := newStrictSyncNode(zapLogger, pebbleDataProofStore, pebbleClockStore, pebbleCoinStore, fileKeyManager, mdbxdb, synchronizer)
	if err != nil {
		return nil, err
	}
	return node, nil
